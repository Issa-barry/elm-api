...
class PackingCrudTest extends TestCase
{
    ...
    public function test_index_isole_les_packings_des_autres_usines(): void
    {
        ...
        // de référence (generateReference() est scopé par usine via HasSiteScope).
        ...
        $response->assertOk();
        $this->assertCount(1, $response->json('data'));
        $this->assertEquals($this->usine->id, $response->json('data.0.site_id'));
    }
    ...
    public function test_store_genere_des_references_uniques_quand_on_change_dusine(): void
    {
        ...
        $this->staff->sites()->attach($autreUsine->id, [
            'role'       => SiteRole::MANAGER->value,
            'is_default' => false,
        ]);
        ...
        Stock::updateOrCreate(
            ['produit_id' => $this->produitRouleau->id, 'site_id' => $autreUsine->id],
            ['qte_stock' => 100, 'seuil_alerte_stock' => 5]
        );

        Sanctum::actingAs($this->staff);

        $responseUsineA = $this->withHeader('X-Site-Id', (string) $this->usine->id)
        ...
        $responseUsineB = $this->withHeader('X-Site-Id', (string) $autreUsine->id)
            ->postJson('/api/v1/packings', [
                'prestataire_id'   => $autrePrestataire->id,
                'date'             => today()->toDateString(),
                'nb_rouleaux'      => 1,
