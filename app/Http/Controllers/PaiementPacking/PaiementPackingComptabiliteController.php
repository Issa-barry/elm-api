...
class PaiementPackingComptabiliteController extends Controller
{
    use ApiResponse;
    ...
    public function __invoke(Request $request)
    {
        try {
            ...
            // 1. Packings non encore facturés (validés sans paiement_id)
            ...
            // 2. Paiements en cours (non soldés)
            $queryPaiements = PaiementPacking::query()->nonSoldes();

            if ($request->has('prestataire_id')) {
                $queryPaiements->where('prestataire_id', $request->prestataire_id);
            }

            $paiementsEnCours = $queryPaiements
                ->select(
                    'prestataire_id',
            ...
            $versementsParPrestataire = DB::table('versements')
                ->join('paiement_packings', 'versements.paiement_packing_id', '=', 'paiement_packings.id')
                ->whereNull('versements.deleted_at')
                ->whereNull('paiement_packings.deleted_at')
                ->when($request->has('prestataire_id'), function ($q) use ($request) {
                    $q->where('paiement_packings.prestataire_id', $request->prestataire_id);
                })
                ->select(
                    'paiement_packings.prestataire_id',
                    DB::raw('SUM(versements.montant) as montant_verse')
                )
                ->groupBy('paiement_packings.prestataire_id')
                ->get()
                ->keyBy('prestataire_id');
            ...
            $result = $prestataireIds->map(function ($prestataireId) use (
                ...
                $enCours = $paiementsEnCours->get($prestataireId);
                ...
                return [
                    ...
                    'nb_paiements_en_cours' => (int) ($enCours->nb_paiements ?? 0),
